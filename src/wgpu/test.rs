...
use crate::{TimeProfile, DetectorConfig, util::{ImageY8, image::ImageRefY8}, detector::{config::{QuadDecimateMode, GpuAccelRequest}, quad_sigma_cpu}, wgpu::{buffer::GpuStage, GpuStageContext, GpuQuadDecimate}, quad_thresh::threshold::threshold};

use super::{WGPUDetector, buffer::GpuImageDownload};
...
fn gpu_quad_decimate(quad_decimate: QuadDecimateMode, src_cpu: &ImageY8) -> ImageY8 {
    ...
    let mut gpu = WGPUDetector::new(&config).unwrap();
    if gpu.quad_decimate.is_none() && quad_decimate == QuadDecimateMode::Scaled(NonZeroU32::new(1).unwrap()) {
        gpu.quad_decimate = Some(GpuQuadDecimate::new_factor(&gpu.context, NonZeroU32::new(1).unwrap()).unwrap());
    }
    ...
    let (cmd_buf, mut dst_gpu) = {
        ...
        let mut ctx = GpuStageContext {
            ...
            cpass: encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: None }),
        };
        println!("C");
    ...
    dst_gpu.index = Some(gpu.context.queue.submit([cmd_buf]));
